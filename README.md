...
├── quickapp/hello_quickapp/     # 快应用作品（通过 manifest <linkfile> 映射到编译树）
│   ├── src/                     # 快应用源码（AIoT-IDE 工程）
│   │   ├── manifest.json
│   │   ├── app.ux
│   │   ├── pages/Index/index.ux # 主页面（index.js 为页面逻辑）
│   │   ├── common/
│   │   │   ├── scripts/         # JS 工具模块
│   │   │   ├── components/      # 自定义组件
│   │   │   └── assets/          # 图标资源
...
│   ├── package.json            # 构建配置（aiot-toolkit）
│   ├── project.config.json     # 项目配置
│   └── docs/                   # 文档
│       └── EmotionSense_Technical_Report_v2.docx
└── logs/                       # AI Coding 日志
    └── AI_Coding_Log.md
```
